// Updated App.js - Enhanced with multi-camera support and scenarios
...
import { 
  ...
  updatePopupSettings,
  getPopupSettings
} from './services/alertPopupManager';
...
function App() {
  ...
  // Load popup settings on startup
  useEffect(() => {
  ...
  // Update popup settings when sound settings change
  useEffect(() => {
    updatePopupSettings({
      ...popupSettings,
      enableSound: soundEnabled
  ...
  // Enhanced alert severity classification with scenario support
  const classifyAlertSeverity = (alert, scenario = 'multi-object') => {
    const confidence = alert.confidence;
    const objectType = alert.objectType.toLowerCase();
    
    // Scenario-based severity classification
    switch (scenario) {
      case 'person-only':
        ...
        return 'info'; // Non-person objects are less critical in person-only scenarios
        
      case 'vehicle-only':
        if (['car', 'truck', 'motorcycle', 'bus'].includes(objectType)) {
      ...
      case 'custom':
        // Custom scenario - you can implement custom logic here
        return classifyCustomSeverity(alert);
        
      case 'multi-object':
      default:
        // Default multi-object classification
  ...
  // Custom severity classification (placeholder for user customization)
  const classifyCustomSeverity = (alert) => {
    // This can be customized based on user preferences
    // For now, use default logic
    const confidence = alert.confidence;
    const objectType = alert.objectType.toLowerCase();
  ...
  // Enhanced alert subscription with popup integration and scenario support
  useEffect(() => {
    ...
    if (isTracking) {
      ...
      unsubscribe = subscribeToAlerts('all', (newAlert) => {
        ...
        // Find the camera pair to get the scenario
        const cameraPair = cameraModelPairs.find(pair => pair.camera.id === newAlert.cameraId);
        const scenario = cameraPair?.scenario || 'multi-object';
        
        const severity = classifyAlertSeverity(newAlert, scenario);
        const enhancedAlert = {
          ...newAlert,
          severity,
          scenario,
  ...
  }, [isTracking, showAlerts, cameras, cameraModelPairs, playAlertSound, flashCamera]);
  ...
  // Enhanced camera-model pair management with scenario support
  const addCameraModelPair = (pair) => {
    console.log('➕ Adding camera-model pair:', pair);
    const enhancedPair = {
      ...pair,
      scenario: pair.scenario || 'multi-object', // Default scenario
  ...
  const handleStartTracking = async () => {
    try {
      ...
      console.log('🎯 Starting enhanced tracking system:', cameraModelPairs.map(p => ({
        camera: p.camera.id,
        model: p.model,
        scenario: p.scenario || 'multi-object'
      })));
      ...
      for (const pair of cameraModelPairs) {
        try {
          console.log(`🚀 Starting tracking for camera ${pair.camera.id} with model ${pair.model} (${pair.scenario || 'multi-object'})`);
          
          // Send scenario information to backend if supported
          const trackingParams = {
            model: pair.model,
            scenario: pair.scenario || 'multi-object'
          };
          
          await startTracking(pair.camera.id, pair.model, trackingParams);
      ...
      if (updatedTrackingPairs.length > 0) {
        ...
        console.log('✅ Enhanced tracking started successfully for cameras:', 
          updatedTrackingPairs.map(p => `${p.cameraId} (${p.model}/${p.scenario})`));
      }
  ...
  // Get camera configuration summary for display
  const getCameraConfigSummary = () => {
    const models = new Set(cameraModelPairs.map(p => p.model));
    const scenarios = new Set(cameraModelPairs.map(p => p.scenario || 'multi-object'));
    
    return {
      totalCameras: cameraModelPairs.length,
      modelsUsed: models.size,
      scenariosUsed: scenarios.size,
      modelBreakdown: Array.from(models).map(model => ({
        model,
        count: cameraModelPairs.filter(p => p.model === model).length
      })),
      scenarioBreakdown: Array.from(scenarios).map(scenario => ({
        scenario,
        count: cameraModelPairs.filter(p => (p.scenario || 'multi-object') === scenario).length
      }))
    };
  };

  const configSummary = getCameraConfigSummary();
  ...
  return (
    <div className={getLayoutClasses()}>
      ...
      <header className="app-header">
        <div className="header-left">
          <div className="system-branding">
            ...
            <span className="system-title">AI Security System</span>
          </div>
          ...
          {configSummary.totalCameras > 0 && (
            <div className="config-summary">
              <span className="config-text">
                {configSummary.totalCameras} cam{configSummary.totalCameras !== 1 ? 's' : ''} | 
                {configSummary.modelsUsed} model{configSummary.modelsUsed !== 1 ? 's' : ''} | 
                {configSummary.scenariosUsed} scenario{configSummary.scenariosUsed !== 1 ? 's' : ''}
              </span>
            </div>
          )}
        </div>
      ...
      <main className="app-main">
        {/* Left Sidebar - Enhanced Control Panel */}
        {showControlPanel && (
